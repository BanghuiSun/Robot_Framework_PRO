...
End_AddDataSource
    ...
    ...    ${dataSourceType} | 数据源类型
    #step3-2 输入共有信息
    输入文本    css=[cc-validate-model="dataSourceOwner.owner"]    ${dataSourceOwnerPlatform}
    输入文本    css=[cc-validate-model="dataSourceOwner.ownerPlatform"]    ${dataSourceOwnerName}
    输入文本    css=[cc-validate-model="dataSourceOwner.ownerTEL"]    ${dataSourceOwnerTEL}
    #step4 确认操作
    点击按钮    确定
