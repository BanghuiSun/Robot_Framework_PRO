...
choose_1_exportInfo_Addjob_hive
    ...
    #输入yarn运行队列-start
    log    ***设置yarn队列***
    输入文本    css=input[name="yarnname"]    banghui    #yarn运行队列
    Sleep    2
    #输入yarn运行队列-end
    #设置分区—start
    Wait Until Page Does Not Contain    该hive表名可用    10    error
    log    ***开始设置分区***
    点击元素    xpath=*//input[@ng-click="choosePart(job.submitParams.hiveOptions['hive-partition-enable'])"]/../i    #勾选分区
    Sleep    0.5
    输入文本    css=input[name="key"]    auto_pt_time    #输入分区字段
    Select From List By Value    css=select[ng-model="job.submitParams.hiveOptions['hive-partition-period']"]    day    #设置分区周期为天
    Sleep    1
    log    ***结束设置分区***
    #设置分区—end
    截图    job-{index}
...
choose_2_exportInfo_Addjob_hbase
    ...
    #输入yarn运行队列-start
    log    ***设置yarn队列***
    输入文本    xpath=*//div[@name='hbase']/div[last()]/div/input    banghui    #yarn运行队列
    Sleep    1
    #输入yarn运行队列-end
    截图    job-{index}
    Sleep    1
